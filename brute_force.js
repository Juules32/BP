...
function solve_password(password_type) {
    ...
    for (let i = minimum_length; i <= maximum_length; i++) {
        ...
        //inderste loop laver random characters fra charSet, indtil der ikke er flere muligheder, hvorefter yderste loop starter forfra
        while (true) {
            guess = ""
            for (let j = 0; j < i; j++) {
            ...
            if(guess == password) {
                ...
                document.getElementById("visual_answer").innerHTML = `Dit password er: ${guess}, tog ${wrong_passwords.length} forsøg og blev løst på ${t2} millisekunder`
            ...
            //'combinations', overstiger mængden af mulige characters, charSet.length løftet til mængden af nuværende cifre
